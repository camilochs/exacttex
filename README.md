...
The article I had in mind when creating ExactTex contains many elements (figures, tables, algorithms, etc.). If you type `\ref{tab:runtime}` in a table (`\label{tab:runtime_1}`), the compiler will flag an error before the PDF prints the wrong word and no one notices, because the `tab:runtime` reference does not exist. The problem was seeing the silent symbol `??` in the PDF. That’s what I wanted to fix.
