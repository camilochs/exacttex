...
pub fn map_emitted_diagnostic(
    ...
    let blame = match origin.kind {
        ...
        OriginKind::XtexNative => Blame::XtexConstruct,
        OriginKind::XtexGenerated => Blame::XtexGenerated,
    };
    MappedDiagnostic {
        blame,
...
fn mapped_offset(output_start: u32, source: Span, kind: OriginKind, output_offset: u32) -> u32 {
    ...
    if kind == OriginKind::XtexGenerated {
        return source_start;
    }
...
mod tests {
    ...
    fn an_error_in_generated_bytes_blames_the_emitter() {
        ...
        assert_eq!(mapped.blame, Blame::XtexGenerated);
        assert!(
            mapped.span.is_some(),
